...
static int rwnx_cfg80211_del_iface(struct wiphy *wiphy, struct wireless_dev *wdev)
{
   ...
   // printk("del_iface: %p\n",wdev);
    ...
    AICWFDBG(LOGINFO, "%s Remove Interface - keeping alive\r\n", dev->name);
    return 0;
}
								  ...
								  struct station_info *sinfo, u8 *phymode, u32 *tx_phyrate, u32 *rx_phyrate)
{
	...
	if (rwnx_send_get_sta_info_req(vif->rwnx_hw, sta->sta_idx, &cfm)) {
		AICWFDBG(LOGINFO, "rwnx_send_get_sta_info_req failed for sta_idx=%d\n", sta->sta_idx);
...
//#endif
    ...
    rwnx_hw->tx_power = 20; /* default 20 dBm */
